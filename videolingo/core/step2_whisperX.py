...
from videolingo.core.all_whisper_methods.demucs_vl import RAW_AUDIO_FILE, VOCAL_AUDIO_FILE, demucs_main
from videolingo.core.all_whisper_methods.whisperX_utils import (
...
from videolingo.core.step1_ytdlp import find_video_files
...
def transcribe_audio(audio_file: str, start: float, end: float) -> Dict:
    ...
    try:
        ...
        result = whisperx.align(
            result["segments"], model_a, metadata, audio_segment, device, return_char_alignments=False
        )
...
def enhance_vocals(vocals_ratio=2.50):
    ...
    try:
        ...
        ffmpeg_cmd = (
            f'ffmpeg -y -i "{VOCAL_AUDIO_FILE}" ' f'-filter:a "volume={vocals_ratio}" ' f'"{ENHANCED_VOCAL_PATH}"'
        )
        subprocess.run(ffmpeg_cmd, shell=True, check=True, capture_output=True)
