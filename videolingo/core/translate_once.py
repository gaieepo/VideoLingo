...
from videolingo.core.prompts_storage import generate_shared_prompt, get_prompt_expressiveness, get_prompt_faithfulness
...
def translate_lines(
    lines, previous_content_prompt, after_cotent_prompt, things_to_note_prompt, summary_prompt, index=0
):
    shared_prompt = generate_shared_prompt(
        previous_content_prompt, after_cotent_prompt, summary_prompt, things_to_note_prompt
    )
    ...
    def retry_translation(prompt, step_name):
        ...
        for retry in range(3):
            if step_name == "faithfulness":
                result = ask_gpt(
                    prompt + retry * " ", response_json=True, valid_def=valid_faith, log_title=f"translate_{step_name}"
                )
