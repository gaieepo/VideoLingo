...
__all__ = ["download_video_ytdlp", "find_video_files"]
    ...
    if cutoff_time:
        ...
        if duration > cutoff_time:
            ...
            process = subprocess.Popen(
                ffmpeg_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True, encoding="utf-8"
            )
...
def find_video_files(save_path="output"):
    ...
    if len(video_files) != 1:
        raise ValueError(
            f"Number of videos found is not unique. Please check. Number of videos found: {len(video_files)}"
        )
