...
from videolingo.core.step4_1_summarize import search_things_to_note_in_prompt
from videolingo.core.step6_generate_final_timeline import align_timestamp
from videolingo.core.translate_once import translate_lines
from videolingo.core.utils import check_len_then_trim
...
def translate_chunk(chunk, chunks, theme_prompt, i):
    ...
    translation, english_result = translate_lines(
        chunk, previous_content_prompt, after_content_prompt, things_to_note_prompt, theme_prompt, i
    )
...
def translate_all():
    ...
    if os.path.exists(TRANSLATION_RESULTS_FILE):
        console.print(
            Panel(
                "🚨 File `translation_results.xlsx` already exists, skipping TRANSLATE ALL.",
                title="Warning",
                border_style="yellow",
            )
        )
