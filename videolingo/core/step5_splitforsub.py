...
from videolingo.core.prompts_storage import get_align_prompt
from videolingo.core.step3_2_splitbymeaning import split_sentence
...
def split_for_sub_main():
    ...
    for attempt in range(3):  # 使用固定的3次重试
        ...
        # 检查是否所有字幕都符合长度要求
        if all(len(src) <= MAX_SUB_LENGTH for src in split_src) and all(
            calc_len(tr) * TARGET_SUB_MULTIPLIER <= MAX_SUB_LENGTH for tr in split_trans
        ):
            break
